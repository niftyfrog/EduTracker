...
const FA_NODES = [
  {
    ...
    tasks: [
      { id: 'fa1', no: 1, title: 'センサーデータのSELECT基礎', tag: '基礎' },
      { id: 'fa2', no: 2, title: 'Oracle接続・クエリ実行', tag: '基礎' },
      { id: 'fa3', no: 3, title: '時系列データのWHERE・ORDER BY', tag: '基礎' },
      { id: 'fa4', no: 4, title: 'リアルタイム集計（GROUP BY）', tag: '実案件' },
      { id: 'fa5', no: 5, title: 'センサー異常検知クエリ設計', tag: '実案件' },
    ],
  },
  {
    ...
    tasks: [
      { id: 'fa6',  no: 1, title: 'C#基礎文法・型システム', tag: '基礎' },
      { id: 'fa7',  no: 2, title: '非同期処理（async/await）', tag: '基礎' },
      { id: 'fa8',  no: 3, title: 'シリアル通信ライブラリ実装', tag: '実案件' },
      { id: 'fa9',  no: 4, title: 'PLCプロトコル（Modbus）実装', tag: '実案件' },
      { id: 'fa10', no: 5, title: '制御ループ・インターロック設計', tag: '実案件' },
    ],
  },
  {
    ...
    tasks: [
      { id: 'fa11', no: 1, title: 'VB.NET基礎・Windowsフォーム', tag: '基礎' },
      { id: 'fa12', no: 2, title: 'タイマー・イベント駆動UI', tag: '基礎' },
      { id: 'fa13', no: 3, title: 'リアルタイムグラフ描画', tag: '実案件' },
      { id: 'fa14', no: 4, title: 'アラーム管理システム実装', tag: '実案件' },
      { id: 'fa15', no: 5, title: '画面遷移・状態管理設計', tag: '実案件' },
    ],
  },
  {
    ...
    tasks: [
      { id: 'fa16', no: 1, title: 'テーブル設計・正規化', tag: '基礎' },
      { id: 'fa17', no: 2, title: 'Oracle固有機能（ROWNUM等）', tag: '基礎' },
      { id: 'fa18', no: 3, title: 'インデックス設計・チューニング', tag: '実案件' },
      { id: 'fa19', no: 4, title: 'ストアドプロシージャ実装', tag: '実案件' },
      { id: 'fa20', no: 5, title: 'バックアップ・リカバリ設計', tag: '実案件' },
    ],
  },
  {
    ...
    tasks: [
      { id: 'fa21', no: 1, title: 'Java + JDBC接続', tag: '基礎' },
      { id: 'fa22', no: 2, title: 'SQL集計結果のJavaへのマッピング', tag: '基礎' },
      { id: 'fa23', no: 3, title: 'JasperReportsでPDF生成', tag: '実案件' },
      { id: 'fa24', no: 4, title: 'Apache POIでExcel出力', tag: '実案件' },
      { id: 'fa25', no: 5, title: '定期レポートスケジューラ実装', tag: '実案件' },
    ],
  },
  {
    ...
    tasks: [
      { id: 'fa26', no: 1, title: 'REST API呼び出し（HttpClient）', tag: '基礎' },
      { id: 'fa27', no: 2, title: 'JSON/XMLシリアライズ', tag: '基礎' },
      { id: 'fa28', no: 3, title: 'OAuth2認証実装', tag: '実案件' },
      { id: 'fa29', no: 4, title: 'Webhookサーバー実装', tag: '実案件' },
      { id: 'fa30', no: 5, title: 'エラーハンドリング・リトライ設計', tag: '実案件' },
    ],
  },
]
...
function FANode({ node, selected, unlocked, progress, onClick, justUnlocked }) {
  ...
  return (
    <g onClick={unlocked ? onClick : undefined} style={{ cursor: unlocked ? 'pointer' : 'default' }}>
      ...
      {complete && (
        <rect x={node.x} y={node.y} width={W} height={H} rx={R}
          fill={node.color} fillOpacity={0.07} stroke={node.color} strokeWidth={2} />
      )}
      ...
      <circle cx={node.x + W - 22} cy={node.y + H - 20} r={ringR} fill="none" stroke="#f0f0f0" strokeWidth={4} />
            ...
            return (
              <div key={task.id} onClick={() => toggleTask(task.id)} style={{
                ...
                <div style={{
                  ...
                  fontSize: 10, color: '#fff', fontWeight: 800, transition: 'all 0.2s',
                }}>{isDone && '✓'}</div>
                <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <div style={{ flex: 1, overflow: 'auto', padding: '24px', position: 'relative', background: '#fafafa' }}>
          ...
          <svg width={SVG_W} height={SVG_H} style={{ display: 'block', minWidth: SVG_W }}>
            <defs>
              ...
              <style>{`
                @keyframes fa-pulse { 0% { r: 50; opacity: 0.6; } 100% { r: 90; opacity: 0; } }
        ...
        <div style={{ width: 320, borderLeft: '1px solid rgba(0,0,0,0.07)', background: '#fff', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '-2px 0 12px rgba(0,0,0,0.04)' }}>
          <FAPanel node={selectedNode} nodeStates={nodeStates} setNodeStates={setNodeStates} />
        </div>
      </div>
    </div>
  )
