...
function App() {
  ...
  // Per-role mutable trainee state
  const [traineeData, setTraineeData] = React.useState(TRAINEES);
  const myTrainee = traineeData[0]; // 山田 花子 = logged-in trainee
  const setMyTrainee = (t) => setTraineeData((d) => d.map((x, i) => i === 0 ? t : x));

  // Sync selectedTrainee from traineeData
  ...
  function renderScreen() {
    switch (screen) {
      case 'dashboard':
        return role === 'admin' ?
        <AdminDashboard trainees={traineeData} setScreen={setScreen} setSelectedTrainee={(t) => {setSelectedTrainee(t);}} setSelectedLang={setSelectedLang} /> :
        <TraineeDashboard trainee={myTrainee} setScreen={setScreen} setSelectedLang={setSelectedLang} />;
      ...
      case 'edit':
        ...
        <TaskEditScreen editTask={editTask} setEditTask={setEditTask} setScreen={setScreen} /> :
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: '#999', textTransform: 'uppercase', letterSpacing: '0.06em' }}>切り替え</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px', borderRadius: 6, background: '#f0f0f0', border: '1px solid rgba(0,0,0,0.07)' }}>
          ...
          <span style={{ fontSize: 12, color: '#666' }}>{currentUser.name}</span>
        </div>
      </div>
