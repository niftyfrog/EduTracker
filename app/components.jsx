...
// Progress bar
function ProgressBar({ value, color = N.green, height = 4 }) {
  return (
...
// Button
function Btn({ children, onClick, variant = 'default', size = 'md', style = {}, disabled }) {
  const [hover, setHover] = React.useState(false);
  const base = {
  ...
  return (
    <input
      ...
      style={{
        ...
        borderRadius: 6, color: N.text, fontSize: 13,
...
function SectionTitle({ children, action }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
      <h2 style={{ fontSize: 13, fontWeight: 500, color: N.muted, textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>{children}</h2>
      {action}
    </div>
  );
}
...
// Stat card
function StatCard({ label, value, suffix = '', color = N.text }) {
  return (
    <Card style={{ flex: 1 }}>
      <div style={{ fontSize: 11, color: N.muted, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>{label}</div>
...
Object.assign(window, { N, ProgressBar, Badge, Card, Btn, Input, Select, Textarea, Avatar, SectionTitle, Divider, StatCard });
