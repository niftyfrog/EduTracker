...
const LANGUAGES = [
  { id: 'java',   name: 'Java',       icon: '☕', total: 10, color: '#f89820' },
  { id: 'python', name: 'Python',     icon: '🐍', total: 10, color: '#4d9fff' },
  { id: 'sql',    name: 'SQL',        icon: '🗃️', total: 5,  color: '#a78bfa' },
  { id: 'js',     name: 'JavaScript', icon: '⚡', total: 8,  color: '#f5c542' },
  { id: 'git',    name: 'Git',        icon: '🌿', total: 5,  color: '#00e599' },
];

const TASKS_DATA = {
  java: [
    { id: 1, no: 1, title: '変数とデータ型', tag: '基礎', desc: 'Javaにおける基本的なデータ型（int, String, boolean等）と変数の宣言方法を学習します。', links: ['https://docs.oracle.com/javase/tutorial/', 'https://qiita.com/java-basics'], comments: [{ who: '田中 先輩', role: 'admin', time: '2日前', text: '公式ドキュメントをしっかり読んでみてください！' }] },
    { id: 2, no: 2, title: '制御フロー（if/for/while）', tag: '基礎', desc: '条件分岐と繰り返し処理の基本を理解し、実際にコードで確認します。', links: ['https://docs.oracle.com/javase/tutorial/java/nutsandbolts/flow.html'], comments: [] },
    { id: 3, no: 3, title: 'オブジェクト指向の基本', tag: '基礎', desc: 'クラス・インスタンス・継承・ポリモーフィズムの概念を理解します。', links: ['https://docs.oracle.com/javase/tutorial/java/concepts/', 'https://qiita.com/oop-guide', 'https://confluence.internal/java-samples'], comments: [{ who: '田中 先輩', role: 'admin', time: '2日前', text: 'まずリンク①を読んでから手を動かしてください！' }, { who: '山田 花子', role: 'trainee', time: '1日前', text: '継承のところが難しかったですが、サンプルコードで理解できました！' }] },
    ...
    { id: 5, no: 5, title: 'コレクションフレームワーク', tag: '基礎', desc: 'List, Map, Setなどのコレクションクラスの使い方を学習します。', links: [], comments: [] },
  ...
  python: [
    ...
    { id: 14, no: 4, title: 'クラスとOOP', tag: '基礎', desc: 'Pythonにおけるオブジェクト指向を理解します。', links: [], comments: [] },
  ...
  sql: [
    ...
    { id: 23, no: 3, title: 'JOIN（結合）', tag: '基礎', desc: 'テーブルの結合方法を学習します。', links: [], comments: [] },
...
// Per-trainee completion state
const TRAINEES = [
  {
    id: 'trainee_001', name: '山田 花子', joined: '2026年4月',
    checked: { java: [1,2,3,4,5,6,7,8], python: [11,12,13,14,15,16,17], sql: [21,22,23], js: [], git: [34,35,36,37,38] },
  },
  {
    id: 'trainee_002', name: '鈴木 太郎', joined: '2026年4月',
    checked: { java: [1,2,3,4,5], python: [11,12,13], sql: [21,22], js: [], git: [34,35,36] },
  },
  {
    id: 'trainee_003', name: '伊藤 美咲', joined: '2026年4月',
    checked: { java: [1,2,3,4,5,6], python: [11,12,13,14,15], sql: [21,22,23], js: [26], git: [34,35,36,37,38] },
  },
];

function getProgress(trainee, langId) {
  const tasks = TASKS_DATA[langId] || [];
  if (!tasks.length) return 0;
  const done = (trainee.checked[langId] || []).length;
  return Math.round((done / tasks.length) * 100);
}

function getOverallProgress(trainee) {
  let total = 0, done = 0;
  LANGUAGES.forEach(l => {
    total += (TASKS_DATA[l.id] || []).length;
    done += (trainee.checked[l.id] || []).length;
  });
  return total ? Math.round((done / total) * 100) : 0;
}

Object.assign(window, { LANGUAGES, TASKS_DATA, TRAINEES, getProgress, getOverallProgress });
