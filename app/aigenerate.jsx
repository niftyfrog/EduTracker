...
function AIGenerateScreen({ role }) {
  ...
  function savePending(tasks) {
    ...
    localStorage.setItem('ai-pending-tasks', JSON.stringify(tasks));
  }
  function saveApproved(tasks) {
    setApprovedTasks(tasks);
    localStorage.setItem('ai-approved-tasks', JSON.stringify(tasks));
  }
...
以下のJSON形式で回答してください（コードブロックなし、JSONのみ）:
{
  ...
  function approveTask(taskId) {
    ...
    if (!task) return;
    const approved = { ...task, status: 'approved', approvedAt: new Date().toLocaleString('ja-JP') };
    saveApproved([approved, ...approvedTasks]);
      ...
      {activeTab === 'generate' && (
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <textarea value={context} onChange={e => setContext(e.target.value)}
                  ...
                  rows={3} style={{
                    ...
                    fontFamily: 'inherit', resize: 'none', color: '#1a1a1a',
                    background: '#fafafa',
                  }} />
              </div>
              ...
              <button onClick={generate} disabled={generating || role !== 'admin'} style={{
                ...
                boxShadow: generating || role !== 'admin' ? 'none' : 'inset 0 1px 0 rgba(255,255,255,0.15), 0 2px 8px rgba(145,22,25,0.3)',
          ...
          <div style={{ flex: 1 }}>
            ...
            {draft && editedDraft && (
              <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, overflow: 'hidden', animation: 'reveal 0.4s ease both' }}>
                ...
                <div style={{ padding: '20px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {editedDraft.checkpoints.map((cp, i) => (
                          <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12, color: '#555' }}>
                            <span style={{ color: '#911619', flexShrink: 0, marginTop: 2 }}>▸</span>
                            <span>{cp}</span>
                          </div>
                        ))}
                      </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, paddingTop: 4, borderTop: '1px solid rgba(0,0,0,0.07)' }}>
                    <button onClick={() => { setDraft(null); setEditedDraft(null); }} style={{
                      flex: 1, padding: '8px', borderRadius: 8, border: '1px solid rgba(0,0,0,0.1)',
                      background: '#fff', color: '#888', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit',
                    }}>破棄</button>
                    ...
                    <button onClick={submitForApproval} style={{
                      ...
                      background: '#911619', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
                      boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.15), 0 2px 6px rgba(145,22,25,0.3)',
                    }}>承認申請する →</button>
                  </div>
                </div>
              </div>
          ...
          {pendingTasks.map(task => (
            <div key={task.id} style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px 18px' }}>
                <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 8, letterSpacing: '-0.01em' }}>{task.title}</div>
                ...
                {task.checkpoints?.length > 0 && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    {task.checkpoints.map((cp, i) => (
                      <div key={i} style={{ fontSize: 12, color: '#555', marginBottom: 3 }}>▸ {cp}</div>
                    ))}
                  </div>
                )}
                {role === 'admin' && (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => rejectTask(task.id)} style={{ flex: 1, padding: '8px', borderRadius: 8, border: '1px solid rgba(255,107,107,0.3)', background: '#fff5f5', color: '#c0392b', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>
                      ✕ 却下
                    </button>
                    <button onClick={() => approveTask(task.id)} style={{ flex: 2, padding: '8px', borderRadius: 8, border: 'none', background: '#911619', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.15), 0 2px 6px rgba(145,22,25,0.3)' }}>
                      ✓ 承認して新人へ公開
                    </button>
                  </div>
                )}
                {role !== 'admin' && <div style={{ fontSize: 11, color: '#aaa' }}>管理者の承認を待っています...</div>}
              </div>
            </div>
          ))}
        </div>
...
Object.assign(window, { AIGenerateScreen });
